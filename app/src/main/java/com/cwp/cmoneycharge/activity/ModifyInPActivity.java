...
public class ModifyInPActivity extends Activity{
	...
	int userid;
	ItypeDAO itypeDAO=new ItypeDAO(ModifyInPActivity.this);
	PtypeDAO ptypeDAO=new PtypeDAO(ModifyInPActivity.this);
	...
	protected void onStart(){
		...
		if (strType.equals("btnoutinfo"))// 如果类型是btnoutinfo
		{
			...
			adapter =new ArrayAdapter<String>(ModifyInPActivity.this,android.R.layout.simple_spinner_item,spdata); //动态生成收入类型列表
		...
		btnEdit.setOnClickListener(new OnClickListener() {// 为修改按钮设置监听事件
			...
			public void onClick(View arg0) {
				...
				if (strType.equals("btnoutinfo"))// 判断类型如果是btnoutinfo
				{
					...
					Toast.makeText(ModifyInPActivity.this, "〖数据〗修改成功！", Toast.LENGTH_SHORT).show();
					gotoback();
		...
		btnDel.setOnClickListener(new OnClickListener() {// 为删除按钮设置监听事件
			...
			public void onClick(View arg0) {
				...
				Toast.makeText(ModifyInPActivity.this, "〖数据〗删除成功！", Toast.LENGTH_SHORT)
						.show();
			}
		});
	...
	private void gotoback(){
		...
		Intent intent=new Intent(ModifyInPActivity.this,MainActivity.class);
